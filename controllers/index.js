...
module.exports = {
		...
		db.User
			...
			.then((user) => {
				if (user.data.isLoggedIn && req.body.loggedIn === 'logout') {
					return db.User
						.updateOne({ _id: user._id }, { loginToken: '', 'data.isLoggedIn': false })
	...
	updateAccount: (req, res, next) => {
		...
		switch (body.route) {
			case 'addprovider':
				route = { $push: { 'data.mediData.doctors': body.provider } };
				break;
			case 'addcontact':
				route = { $push: { 'data.emergencyContacts': body.contact } };
				break;
			case 'addsymptom':
				route = { $push: { 'data.symptomHistory': body.symptom } };
				break;
			case 'addinsurance':
				route = { $push: { 'data.mediData.insurance': body.insurance } };
				break;
			case 'updateinsurance':
				route = { $set: body.insurance };
				break;
			case 'updatecontact':
				route = { $upsert: body.contact };
				break;
			case 'updateprovider':
				route = { $upsert: body.provider };
				break;
			case 'updatesymptom':
				route = { $upsert: body.symptom };
				break;
			case 'deletesymptom':
				route = { $pull: { 'data.symptomHistory': { _id: body.symptomId } } };
				break;
			case 'deleteinsurance':
				route = { $pull: { 'data.mediData.insurance': { _id: body.insuranceId } } };
				break;
			case 'deletecontact':
				route = { $pull: { 'data.emergencyContacts': { _id: body.contactId } } };
				break;
			case 'deleteprovider':
				route = { $pull: { 'data.mediData.doctors': { _id: body.providerId } } };
				break;
		}
		...
		if (body.route.substring(0, 3) === 'add') {
			return db.User
				.findOneAndUpdate({ _id: req.body.userId }, route, { new: true })
				.then((data) => {
					// console.log(data)
					if (!data) {
						return;
			...
			return db.User
				.findOneAndUpdate({ [body.key]: body.id }, route, { new: true })
				.then((data) => {
					console.log(data);
					if (!data) {
						return;
			...
			return db.User
				.findOneAndUpdate({ _id: body.userId }, route, { new: true })
				.then((data) => {
	...
	updateProfile: (req, res, next) => {
		...
		db.User
			.findOneAndUpdate({ _id: req.params.userId }, { $upsert: req.body }, { new: true })
			...
			.then((user) => {
				if (!user) {
					return db.User
						...
						.then((newUser) => {
							db.User
								.findOneAndUpdate({ _id: newUser._id }, { loginToken: token }, { new: true })
								.then((token) => {
									res.json({
										userId: token._id,
										user: token.data,
				...
				db.User
					.findOneAndUpdate({ _id: user._id }, { loginToken: token, 'data.isLoggedIn': true }, { new: true })
					.then((user) => {
						res.json({
							userId: user._id,
							user: user.data,
